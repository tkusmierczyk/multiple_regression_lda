...
worddist  = apply(samples$worddist[1:K, 1:length(train.words), , 1], c(1,2), approx.mode)
weight    = apply(samples$weight[1:K, , 1], c(1), approx.mode)
intercept = approx.mode(samples$intercept)
sd        = approx.mode(samples$sd)
...
predicted.out = as.data.frame(apply(samples$out[,  , 1], c(1), approx.mode))
